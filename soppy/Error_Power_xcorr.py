...
"""
This code culculate the cross-corrlation or cross-correlation 
between error and power at different frequency range.
...
i = np.linspace(0, len(error2), np.size(power,1), endpoint=True)
x = range(0, len(error2))
y = error2.values
...
#prepare to plot
ti = np.linspace((startT-endT+windowLg)*c/(np.size(stimes,1)-1),(endT-startT-windowLg)*c/(np.size(stimes,2)-1), len(epxcov[1,:]))
